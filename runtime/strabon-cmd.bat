...
if "%CLASS%"=="StoreOp" (
    ...
    :ready_file
    set STRABON_EXEC=java %JAVA_OPTS% -cp ;target\* %PKG%.%DATABASE%.%CLASS% %HOST% %PORT% %DB% %DBUSER% %DBPASS% "%FILE%" -f %FORMAT% %NAMED_GRAPH% %INFERENCE%
    goto :execute
)
...
if "%CLASS%"=="QueryOp" (
    set STRABON_EXEC=java %JAVA_OPTS% -cp ;target\* %PKG%.%DATABASE%.%CLASS% %HOST% %PORT% %DB% %DBUSER% %DBPASS% "%PREFIXES% %QUERY%" %FORCE_DELETE% %RESULT_FORMAT%
    goto :execute
)

:update_or_describe_exec
set STRABON_EXEC=java %JAVA_OPTS% -cp ;target\* %PKG%.%DATABASE%.%CLASS% %HOST% %PORT% %DB% %DBUSER% %DBPASS% "%PREFIXES% %QUERY%" %RESULT_FORMAT%
goto :execute    
